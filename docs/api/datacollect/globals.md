[**ID PASS Data Collect API Documentation v0.0.1**](README.md)

***

# ID PASS Data Collect API Documentation v0.0.1

## Enumerations

- [EntityType](enumerations/EntityType.md)
- [SyncLevel](enumerations/SyncLevel.md)

## Classes

- [EntityDataManager](classes/EntityDataManager.md)
- [EntityStoreImpl](classes/EntityStoreImpl.md)
- [EventStoreImpl](classes/EventStoreImpl.md)
- [ExternalSyncManager](classes/ExternalSyncManager.md)
- [InternalSyncManager](classes/InternalSyncManager.md)
- [SyncAdapterImpl](classes/SyncAdapterImpl.md)
- [EventApplierService](classes/EventApplierService.md)
- [IndexedDbEntityStorageAdapter](classes/IndexedDbEntityStorageAdapter.md)
- [IndexedDbEventStorageAdapter](classes/IndexedDbEventStorageAdapter.md)
- [PostgresEntityStorageAdapter](classes/PostgresEntityStorageAdapter.md)
- [PostgresEventStorageAdapter](classes/PostgresEventStorageAdapter.md)

## Interfaces

- [EntityDoc](interfaces/EntityDoc.md)
- [EntityPair](interfaces/EntityPair.md)
- [GroupDoc](interfaces/GroupDoc.md)
- [IndividualDoc](interfaces/IndividualDoc.md)
- [DetailEntityDoc](interfaces/DetailEntityDoc.md)
- [DetailGroupDoc](interfaces/DetailGroupDoc.md)
- [FormSubmission](interfaces/FormSubmission.md)
- [AuditLogEntry](interfaces/AuditLogEntry.md)
- [EventStore](interfaces/EventStore.md)
- [EventStorageAdapter](interfaces/EventStorageAdapter.md)
- [EventApplier](interfaces/EventApplier.md)
- [EncryptionAdapter](interfaces/EncryptionAdapter.md)
- [ExportImportManager](interfaces/ExportImportManager.md)
- [EntityStore](interfaces/EntityStore.md)
- [EntityStorageAdapter](interfaces/EntityStorageAdapter.md)
- [SyncAdapter](interfaces/SyncAdapter.md)
- [AuthenticatedSyncAdapter](interfaces/AuthenticatedSyncAdapter.md)
- [SyncStatus](interfaces/SyncStatus.md)
- [MerkleTreeStorage](interfaces/MerkleTreeStorage.md)
- [Conflict](interfaces/Conflict.md)
- [ExternalSyncAdapter](interfaces/ExternalSyncAdapter.md)
- [ExternalSyncCredentials](interfaces/ExternalSyncCredentials.md)

## Type Aliases

- [ImportResult](type-aliases/ImportResult.md)
- [SearchCriteria](type-aliases/SearchCriteria.md)
- [ExternalSyncConfig](type-aliases/ExternalSyncConfig.md)
